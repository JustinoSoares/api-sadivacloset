...
describe('AuditoriaService', () => {
  ...
  it('registar deve criar log com adminId, acao, entidade, entidadeId, detalhes', async () => {
    ...
    const result = await service.registar('admin1', 'criar_produto', 'produto', 'prod1', {
      nome: 'Teste',
    });
  ...
  it('register alias deve delegar para registar', async () => {
    ...
    const result = await service.register('admin1', 'atualizar_pedido', 'pedido', 'order1', {
      de: 'pago',
    });
  ...
  it('listar deve paginar e filtrar por entidade e intervalo', async () => {
    ...
    expect(prisma.auditLog.count).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ entity: 'produto' }) }),
    );
