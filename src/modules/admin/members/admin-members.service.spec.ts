...
describe('AdminMembersService', () => {
  ...
  it('should give 404 if member not exists', async () => {
    ...
    await expect(service.findOrdersByMember('fake-id', { page: 1, limit: 10, skip: 0, take: 10 } as any)).rejects.toThrow();
  });

  it('legacy alias findPedidosByMembro should work', async () => {
    prisma.user.findUnique.mockResolvedValue({ id: 'u1', role: Role.BUYER });
    prisma.order.count.mockResolvedValue(1);
    prisma.order.findMany.mockResolvedValue([{ id: 'o1', buyerId: 'u1', total: 5000, status: OrderStatus.PAID, createdAt: new Date(), items: [], delivery: null, payment: null }]);
    const dto: any = { page: 1, limit: 10, skip: 0, take: 10 };
