...
describe('AdminAccountService', () => {
  ...
  it('PATCH should require currentPassword to change password', async () => {
    prisma.user.findUnique.mockResolvedValue(adminMock);
    await expect(service.updateAccount(adminId, { newPassword: 'NewPass123!' } as any)).rejects.toBeInstanceOf(BadRequestException);
  });

  it('PATCH should fail if currentPassword incorrect', async () => {
    prisma.user.findUnique.mockResolvedValue(adminMock);
    await expect(service.updateAccount(adminId, { currentPassword: 'WrongPass', newPassword: 'NewPass123!' })).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('PATCH should change password when current correct', async () => {
    ...
    const result = await service.updateAccount(adminId, { currentPassword: 'OldPass123!', newPassword: 'NewPass123!' });
  ...
  it('PATCH should fail if email already exists', async () => {
    ...
    await expect(service.updateAccount(adminId, { email: 'taken@a.ao' })).rejects.toBeInstanceOf(ConflictException);
  });
});
